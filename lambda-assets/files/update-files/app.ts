...
export async function handler(event: { [key: string]: any }) {
  ...
  try {
    ...
    for (let index = 0; index < validateFile.length; index += 1) {
      const existsFiles = await ddbDocClient.send(
        new QueryCommand({
          ...
          ExpressionAttributeValues: {
            ':checksum': validateFile[index].checksum,
            ':version': validateFile[index].version,
          },
        }),
      );
    ...
    new BatchWriteCommand({
      RequestItems: {
        [`${process.env.FILE_TABLE_NAME}`]: updateFileList,
      },
    });
